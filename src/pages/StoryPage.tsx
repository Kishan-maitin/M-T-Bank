...
import {
  saveStoryInteraction,
  getStoryForUser,
  archiveStory,
} from "@/apis/commonApiCalls/storyApi";
...
export default function StoryPage() {
    ...
    const initializeStories = async () => {
      ...
      if (location.state) {
        ...
        if (passedStories && passedStories.length > 0) {
          ...
          const firstUnseenIndex = formattedStories[
            userIndex
          ]?.stories.findIndex((story: StoryItem) => story.seen === 0);
  ...
  useEffect(() => {
    if (!isLoading && allStories.length > 0) {
      const currentStoryItem =
        allStories[currentUserIndex]?.stories[currentStoryIndex];
      if (currentStoryItem) {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") {
        ...
        // Only toggle playback if input is not focused
  ...
  const handleDeleteStory = async () => {
    ...
    if (response.success) {
      ...
      if (updatedStories[currentUserIndex].stories.length === 0) {
        ...
        const newUserIndex =
          currentUserIndex >= updatedStories.length
        ...
        const newStoryIndex =
          currentStoryIndex >= updatedStories[currentUserIndex].stories.length
          ...
          // Check if menu was removed
            ...
            if (!showViewers) {
              ...
              const currentStoryItem =
                allStories[currentUserIndex]?.stories[currentStoryIndex];
              if (
                currentStoryItem?.contentType === "video" &&
                videoRef.current
              ) {
                videoRef.current
                  .play()
                ...
                <div
                  className={`h-full bg-primary transition-all duration-300 ${
                    index < currentStoryIndex
                      ? "w-full"
                      : index === currentStoryIndex && !isVideo
                  ...
                  style={{
                    width:
                      index === currentStoryIndex && isVideo
                        ...
                        : undefined,
                    animationDuration:
                      index === currentStoryIndex && !isVideo ? "5s" : "0s",
