...
const EditProfilePage: React.FC = () => {
  ...
  const { username, email, avatar, profilePic, interests, privacyLevel, bio } =
  ...
  const [customProfilePicPreview, setCustomProfilePicPreview] = useState<
    string | null
  >(profilePic && profilePic !== avatar ? profilePic : null);
  ...
  useEffect(() => {
    const getAvatars = async () => {
      ...
      if (result.success && result.data) {
        ...
        if (avatar) {
          const isMaleAvatar = male.some(
            (item: AvatarData) => item.url === avatar
          );
          const isFemaleAvatar = female.some(
            (item: AvatarData) => item.url === avatar
          );
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-end">
              <span
                className={`text-xs ${
                  formData.bio.length > CHARACTER_LIMIT
                    ? "text-destructive"
