...
const VoiceSettingsPage: React.FC = () => {
  ...
  const { selectedVoice, voiceOptions } = useAppSelector(
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium">
          ...
          />{" "}
          Bond Chat Voice Settings
        </h3>
      </div>

      <p className="text-muted-foreground">
        Choose a voice that will be used when your messages are read aloud in
        Bond Chat.
      </p>
        ...
        {voiceOptions.map((voice) => (
          ...
          >
            <div className="flex items-center gap-2">
              <div
                className={`w-4 h-4 rounded-full ${currentVoice === voice.id ? "bg-primary" : "border border-muted-foreground"}`}
              ></div>
              <span
                className={`font-medium ${currentVoice === voice.id ? "text-primary" : ""}`}
              >
                {voice.name}
              </span>
            </div>
      ...
      <div className="mt-6 p-4 bg-muted rounded-md">
        ...
        <p className="text-sm text-muted-foreground">
          Bond Chat uses text-to-speech technology to read messages aloud. Your
          selected voice will be used when your messages are read to other
          users. This feature can be disabled in the Bond Chat settings.
        </p>
      </div>
    </div>
  );
