...
const AvailableInterestItem = ({
  ...
  return (
    ...
    >
      ...
      <span ref={textRef} className="truncate ml-2">
        {interest}
      </span>
    </button>
  );
...
const SkillsInterestsTab: React.FC<SkillsInterestsTabProps> = ({
  onValidationChange,
}) => {
  const dispatch = useAppDispatch();
  const { skillSelected } = useAppSelector((state) => state.createProfile);
