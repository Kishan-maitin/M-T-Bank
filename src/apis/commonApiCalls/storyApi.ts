...
export const uploadStory = async (
  ...
  formData.append(
    "contentType",
    primaryStory.type === "photo"
      ? "image"
      : primaryStory.type === "video"
        ? "video"
        : "text"
  );
...
export function validateFormData(formData: FormData): boolean {
  ...
  const hasRequiredFields =
    keys.includes("contentType") && keys.includes("privacy");
 ...
 */
export const saveStoryInteraction = async (
  storyId: string
  ...
  if (response.status === 200) {
    ...
    throw new Error(
      response.data.message || "Failed to save story interaction"
    );
  }
};
